...
if [[ "$OS" = "CentOs" && ("$VER" = "6" || "$VER" = "7" ) || 
      "$OS" = "Ubuntu" && ("$VER" = "12.04" || "$VER" = "14.04" ) || "$OS" = "Fedora" && "$VER" = "22" ]] ; then 
    echo "Ok."
else
...
if [[ "$OS" = "CentOs" ]]; then
    ...
    $PACKAGE_INSTALLER sudo vim make zip unzip chkconfig bash-completion
    $PACKAGE_INSTALLER ld-linux-aarch64.so.1 libbz2.so.1 libdb-5.3.so libgd.so.3
    $PACKAGE_INSTALLER curl curl-devel perl-libwww-perl libxml2 libxml2-devel zip bzip2-devel gcc gcc-c++ at make
    $PACKAGE_INSTALLER redhat-lsb-core
elif [[ "$OS" = "Ubuntu" ]]; then
...
if [[ "$OS" = "CentOs" ]]; then
	...
	$PACKAGE_INSTALLER "$DB_PCKG-server"
    MY_CNF_PATH="/etc/my.cnf"
	DB_SERVICE="mariadb"
elif [[ "$OS" = "Ubuntu" ]]; then
...
if [[ ! -d /var/lib/mysql ]] ; then 
	echo "$DB_PCKG-server not installed, please installing $DB_PCKG-server"
	exit 0
fi
...
# Register mysql/mariadb service for autostart on fedora 22 server
if [[ "$OS" = "Fedora" ]]; then
    if [[ "$VER" == "22" ]]; then
        systemctl enable "$DB_SERVICE".service
    else
...
if [[ ("$OS" = "CentOs" && "$VER" = "7") || 
      ("$OS" = "Ubuntu" && "$VER" = "14.04") || ("$OS" = "Fedora" && "$VER" = "22") ]] ; then 
...
if [[ "$OS" = "CentOs" || ( "$OS" = "Ubuntu" && "$VER" = "14.04") || "$OS" = "Fedora" ]] ; then
    echo -e "\n# Building suhosin"
    if [[ "$OS" = "Ubuntu" ]]; then
...
if [[ ("$OS" = "CentOs" && "$VER" == "7") || ("$OS" = "Fedora" && "$VER" = "22") ]]; then
